...
static void racing_penguin_act_race(void) {
    if (cur_obj_follow_path() == PATH_REACHED_END) {
        ...
        f32 targetSpeed = o->oPosY - gPlayerObject->oPosY;
        f32 minSpeed = 70.0f;
...
static void racing_penguin_act_show_final_text(void) {
    if (o->oRacingPenguinFinalTextbox == 0) {
        ...
        s32 textResult;
        if ((textResult = cur_obj_update_dialog_with_cutscene(PLAYER_DIALOG_LOOK_UP,
...
void bhv_racing_penguin_update(void) {
    ...
    switch (o->oAction) {
        case RACING_PENGUIN_ACT_WAIT_FOR_MARIO:
            racing_penguin_act_wait_for_player();
            break;
    ...
    cur_obj_push_player_away_from_cylinder(sRacingPenguinData[o->oBhvParams2ndByte].radius,
