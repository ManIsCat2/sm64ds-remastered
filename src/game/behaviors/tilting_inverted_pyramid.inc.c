 ...
 * The object essentially just tilts and moves Player with it.
 */
...
/**
 * Main behavior for the tilting pyramids in LLL/BitFS. These platforms calculate rough normals from Player's position,
 * then gradually tilt back moving Player with them.
 */
void bhv_tilting_inverted_pyramid_loop(void) {
    Vec3f targetNormal;
    Mat4 *transform = &o->transform;
    s32 playerOnPlatform = (gPlayerObject->platform == o);
    ...
    if (playerOnPlatform) {
        ...
        // Target normal is directly upwards when Player is not on the platform.
    ...
    // Outside of the other conditionals since it needs to tilt regardless of whether Player is on.
