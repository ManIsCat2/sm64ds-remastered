...
void bhv_punch_tiny_triangle_loop(void) {
    if (o->oTimer == 0) {
        ...
        cur_obj_set_pos_relative(gPlayerObject, 0.0f, 60.0f, 100.0f);
...
void bhv_punch_tiny_triangle_init(void) {
    ...
    for (i = 0; i < 6; i++) {
        ...
        triangle->oMoveAngleYaw = gPlayerObject->oMoveAngleYaw + sTinyTriMovementParams[2 * i] + 0x8000;
...
void bhv_wall_tiny_star_particle_loop(void) {
    if (o->oTimer == 0) {
        ...
        cur_obj_set_pos_relative(gPlayerObject, 0.0f, 30.0f, 110.0f);
        o->oMoveAngleYaw = sp1E;
    }
...
void bhv_tiny_star_particles_init(void) {
    ...
    for (i = 0; i < 7; i++) {
        ...
        particle->oMoveAngleYaw = gPlayerObject->oMoveAngleYaw + sTinyStarMovementParams[2 * i] + 0x8000;
