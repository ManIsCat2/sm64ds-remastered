...
u8 sUnusedGotGlobalCoinHiScore = FALSE;
...
u32 save_file_get_cannon_flags(s32 fileIndex, s32 courseIndex) {
    return gSaveBuffer.files[fileIndex][0].courseStars[courseIndex] & (1 << 7);
}
...
void get_complete_save_file(s16 saveNum) {
    int i, j;
    // Apply all stars flags, including cannon
